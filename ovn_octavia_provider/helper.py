        ...
        if delete:
            ...
            for member_id in [item[3] for item in other_members
                ...
                if member_statuses.get(member_id, '') != constants.NO_MONITOR:
                    ...
                    LOG.debug(f"Not removing IP mapping for {backend_ip}: "
                              f"still used by member {member_id}")
                    break
            if execute_delete:
                ...
                if not hm_source_ip:
                    LOG.error("No port on subnet %(subnet)s available for "
                              ...
                              {'subnet': mb_subnet,
                               ...
                               'pool': pool_key})
                    return None
                self._update_ip_port_mappings(ovn_lb, backend_ip, member_lsp.name, hm_source_ip, pool_key, delete)
